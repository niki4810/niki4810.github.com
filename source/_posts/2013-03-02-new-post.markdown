...
To illustrate this, we will be building a simple application which looks like this: [Link](http://jsfiddle.net/niki4810/CSyAz/embedded/result/). The application contains two views, an `editor view` for entering information and a `viewer view` for previewing the same information in read only mode. Both these view display the following information :
...
1. As we update a field on the edit view, the corresponding field on the preview view should be updated
2. When we update the `salary` field, the view should format the value as money
3. When the `favorite search engine` field is changed on the edit view, the preview view should render a hyperlink and should update both the `label` and the `href` properties on that hyperlink.


Now that we have out requirements, As a first step we create a `Backbone Model` and set some default values in it.
...
Create a converter function , that formats the given value as money, for example 123 gets converted to $123.00. This function takes two parameters `value` the amount that needs to be formatted & `direction`, which has two values `ModelToView` or `ViewToModel`. Both these values are automatically supplied by the model binder framework when a change occurs on the binded element.
...
For each view we pass in a bindings object, which determines, which element should be bound to which property on the model. For example, to bind the `firstName` property on the model to an element with `name set to firstName` on the edit view, the binding map would look like.  
...
The literal value `firstName` points to the `key` on the model, and the `[name="firstName"]` acts a selector for selecting an element on our Backbone View.

### Using the `converter` function

Similarly to firstName, we apply set other bindings on the object as well
...
As you might have already noticed, for the `salary` attribute we set the value as a object with internally has two properties. `selector` property determines which element this property should bound to, in this case an element with `name set to salary`. We assign the `salaryConverter` function to the `converter` property. So whenever there is a change event on element with `[name = "salary"]`, the Backbone ModelBinder plugin calls the `salaryConverter` twice. Once from `Model to view` and the other from `View to Model`. Through this we can achieve our second requirement where we want to format the salary attribute as money when displaying on the view.
...
The complete source code for this example can be found at the following [fiddle](http://jsfiddle.net/niki4810/CSyAz/). Although this looks like a lot of code, it simple once we understand the basic concept behind model binding.
